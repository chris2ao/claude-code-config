...
CRITICAL: NEVER include secret values in vector memory. This means:
...
1. **Store to vector memory**: Save a comprehensive summary of the current session to vector memory using `mcp__vector-memory__memory_store`. Include:
   ...
   If vector memory is unavailable (MCP server down, timeout, or error), warn the user explicitly: "Vector memory save failed. Durable context will not be preserved." Do not silently skip.

2. **Confirm to user**: Tell the user: "Context preserved to vector memory. Run /compact now."
...
(The session-state scratchpad step was removed 2026-07-20 in the P1 simplification; vector memory is the single durable pre-compact store.)
