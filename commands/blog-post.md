...
## Phase 3.5: Cover Graphic

Every post gets a cover infographic. **Default pipeline: custom HTML brand graphics.** Use NotebookLM ONLY if the user explicitly asked for NotebookLM in this session; otherwise do not offer it.

1. Spawn the brand-graphics agent (subagent_type: "general-purpose", model: "sonnet"):
   - Prompt: "Follow the instructions in ~/.claude/agents/brand-graphics.md. Blog post: <post-path>. Type: cover. Output mode: repo."
   - The agent authors HTML in the site's brand tokens, renders 2752x1536 with headless Chrome, verifies dimensions and crop-safe zones, writes `public/blog/<slug>/infographic.png` plus the HTML source under `content-assets/covers/<slug>/`, and adds `coverImage`/`coverImageAlt` frontmatter.
2. Verify its report: dimensions exactly 2752x1536, frontmatter fields present, and view the PNG yourself before proceeding.
3. If the user explicitly requested NotebookLM instead: invoke `/notebooklm-content` and expect manual QA for garbled small text (its known failure mode).
...
**Production posts:**
```bash
git add src/content/blog/[new-post].mdx src/content/blog/[previous-post].mdx public/blog/[new-post-slug]/
