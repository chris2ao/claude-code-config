...
- User says `/notebooklm-content` or explicitly names NotebookLM
- User asks for slide decks (slides remain NotebookLM territory)

**This is the BACKUP image pipeline.** For infographics and article graphics, the default is `/brand-graphics` (hand-authored HTML rendered with headless Chrome). Use NotebookLM for images only when the user explicitly says to use NotebookLM. Known failure mode driving this: NotebookLM garbles small text (repo URLs, mockup labels, badge microcopy) and ignores orientation instructions across revision cycles (verified 2026-07-19, five candidates).
