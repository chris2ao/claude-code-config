...
- **Lessons Learned** near the end as a short in-voice prose paragraph, not a stack of bolded cards (see the de-slop check in the voice profile)
- **No manual series navigation footer.** The site renders `BlogSeriesNav` automatically from `series` + `seriesOrder` frontmatter
...
The complete schema (required and optional fields, series rules, schemaType, coverImage) lives in `~/.claude/skills/blog-mdx-reference.md`. That file is the single source of truth; do not duplicate it here.

Style rules on top of the schema:
